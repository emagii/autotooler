...
static char *at_symbol_printer(const char *const in)
{
                        ...
                        if (e && e->left.sym->name) {
                            if (!done) {
                                fprintf(out, "%s: $(BASE_TARGETS)",
                                        at_symbol_printer(sym->name));
                                done = 1;
                            }
//printf("SELECTS %s\n",e->left.sym->name);
                            fprintf(out, " %s",
                                    at_symbol_printer(e->left.sym->name));
                        }
                    }
                    if (done)
                        fprintf(out, "\n");
